...
- JSX 结构中使用花括号{}插入表达式即插值表达式（注意只能是表达式，for循环之类的代码段就无法插入）
...
- 可以在JSX中嵌入任意js表达式内容( 包括 函数，jsx 结构，变量，对象，表达计算式 ) ，字符串、数字正常输出，布尔值、unll空、undefined未定义输出空字符，插值表达式中不能直接输出对象，但是数组对象则可以，注意：React对数组进行转字符串操作，并且是使用空字符串进行连接：arr.join('')
...
- JSX不允许没有闭合的标签；
- 因为 JSX 的特性更接近 js 而不是 HTML , 所以 XML 使用 camelCase 小驼峰命名 来定义属性的名称，而不是使用 HTML 的属性名称;例如：class 用 className 代替，而 tabindex 用 tabIndex 代替；XML可以包含子元素，但是结构中只能有且仅有一个顶级元素
